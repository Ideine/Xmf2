...
namespace Xmf2.Rest.HttpClient.Impl.Http
{
	...
	public class DefaultHttpContent : IHttpContent
	{
		...
		/// </returns>
		public Task LoadIntoBufferAsync(long maxBufferSize) => Content.LoadIntoBufferAsync(maxBufferSize);

		/// <summary>
		/// Returns the data as a stream
		/// </summary>
		/// <returns>The task that returns the stream</returns>
		public Task<Stream> ReadAsStreamAsync() => Content.ReadAsStreamAsync();

		/// <summary>
		/// Returns the data as byte array
		/// </summary>
		/// <returns>The task that returns the data as byte array</returns>
		public Task<byte[]> ReadAsByteArrayAsync() => Content.ReadAsByteArrayAsync();

		/// <summary>
		/// Returns the data as string
		/// </summary>
		/// <returns>The task that returns the data as string</returns>
		public Task<string> ReadAsStringAsync() => Content.ReadAsStringAsync();
