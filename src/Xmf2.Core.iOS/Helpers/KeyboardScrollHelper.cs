...
namespace Xmf2.Core.iOS.Helpers
{
	public class KeyboardScrollHelper : IDisposable
	{
		...
		private readonly WeakReference<UIView> _lastActiveView = new(null);
		...
		private void OnKeyboardChanged(bool visible, CGRect keyboardFrame)
		{
			...
			else
			{
				...
				if (_lastKeyboardFrame == keyboardFrame && _lastActiveView.TryGetTarget(out var lastActiveView) && lastActiveView?.Equals(activeView) == true)
				{
					return;
				}
