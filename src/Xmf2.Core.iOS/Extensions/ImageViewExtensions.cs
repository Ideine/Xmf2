...
namespace Xmf2.Core.iOS.Extensions
{
	public static class ImageViewExtensions
	{
		...
		{
			int height = (int?)view.Constraints.FirstOrDefault(x => x.FirstAttribute == NSLayoutAttribute.Height && x.Relation is NSLayoutRelation.Equal or NSLayoutRelation.LessThanOrEqual)?.Constant ?? 0;
			int width = (int?)view.Constraints.FirstOrDefault(x => x.FirstAttribute == NSLayoutAttribute.Width && x.Relation is NSLayoutRelation.Equal or NSLayoutRelation.LessThanOrEqual)?.Constant ?? 0;
